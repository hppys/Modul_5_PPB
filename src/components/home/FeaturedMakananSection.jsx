...
export default function FeaturedMakananSection({ recipes, loading, error, onRecipeClick, onNavigate }) {
  ...
  useEffect(() => {
    // Pastikan untuk me-reset refs array saat data resep berubah
    makananRefs.current = makananRefs.current.slice(0, recipes?.length || 0);
  ...
  }, [recipes]); // Tambahkan recipes sebagai dependency
    ...
    <section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
        {recipes.map((recipe, index) => (
          ...
          >
            <div
              onClick={() => onRecipeClick && onRecipeClick(recipe.id, 'makanan')}
