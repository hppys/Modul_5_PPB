...
export default function EditRecipePage({ recipeId, onBack, onSuccess }) {
        ...
        if (result.success) {
          ...
          if (recipe.steps && recipe.steps.length > 0) {
            stepsArray = recipe.steps.map(step => {
              // API mengembalikan steps sebagai objek { id, step_number, instruction }
              // Kita hanya butuh string instruction untuk form
              if (typeof step === 'object' && step.instruction) {
                return step.instruction;
              }
              // Fallback jika datanya string (meskipun API doc tidak bilang begitu)
  ...
  // Submit form (PUT - full replacement)
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      updateData.steps = validSteps; // API PUT mengharapkan array of strings
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-6 md:p-8 shadow-xl border border-white/40">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              ...
              {imagePreview ? (
                <div className="relative">
                  ...
                  <button type="button" onClick={handleRemoveNewImage} className="absolute top-4 right-4 p-2 bg-red-500 text-white rounded-full hover:bg-red-600 transition-colors shadow-lg">
                    <X className="w-5 h-5" />
                  </button>
                ...
                /* Show current image */
                <div className="relative">
                  <img src={currentImageUrl} alt="Current" className="w-full h-64 object-cover rounded-xl" />
                  <button type="button" onClick={handleRemoveOriginalImage} className="absolute top-4 right-4 p-2 bg-red-500 text-white rounded-full hover:bg-red-600 transition-colors shadow-lg">
                    <X className="w-5 h-5" />
                  </button>
                  <div className="absolute bottom-4 left-4">
                    <input type="file" accept="image/jpeg, image/jpg, image/png, image/webp" onChange={handleImageChange} className="hidden" id="image-change" />
                    <label htmlFor="image-change" className="px-4 py-2 bg-white/90 backdrop-blur-sm text-slate-700 rounded-full hover:bg-white transition-colors cursor-pointer text-sm font-medium shadow-lg inline-flex items-center gap-2">
                ...
                <div className="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center hover:border-blue-400 transition-colors">
                  <input type="file" accept="image/jpeg, image/jpg, image/png, image/webp" onChange={handleImageChange} className="hidden" id="image-upload" />
            ...
            {/* ... Sisa Form (Basic Info, Description, dll) ... */}
            {/* Basic Info */}
